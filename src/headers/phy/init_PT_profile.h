...
void bilinear_interp(double  xval,
                     double  yval,
                     double  x1,
                     double  x2,
                     double  y1,
...
void bilinear_log_interp(double  xval,
                         double  yval,
                         double  x1,
                         double  x2,
                         double  y1,
...
void bilinear_interpolation_polynomial_fit(double  x,
                                           double  y,
                                           double  x1,
                                           double  x2,
                                           double  y1,
    ...
    a00 = 1.0 / ((x2 - x1) * (y2 - y1))
          * (x2 * y2 * z11 - x2 * y1 * z12 - x1 * y2 * z21 + x1 * y1 * z22);
...
void linear_interp(double Xin, double x1, double x2, double y1, double y2, double& Yout) {
...
void linear_interpolation_fit(int     len,
                              double  Xin,
                              double* Xreference,
                              double* Yreference,
                              double& Yout) {
...
void k_Ross_Freedman_bilinear_interpolation_polynomial_fit(double  Tin,
                                                           double  Pin,
...
void create_pressure_layers(int i, int nlay, double*(&pl), double P_Ref) {
    ...
    double b[] = {2.2727272727272727e-09, 3.3324801199555632e-09, 4.886386449955581e-09,
                  ...
                  0.6819927474188744,     1.0000000000000009};
...
void gam_Parmentier(double Teff,
                    int    table_num,
                    double (&gam_V)[3],
                    double (&Beta_V)[3],
                    double (&Beta)[2],
...
//     Teff0 = pow((pow(Tint, 4.0) + mu * pow(Tirr, 4.0)), (1.0 / 4.0));
...
//     gam_Parmentier(Teff, table_num, gam_V, Beta_V, Beta, gam_1, gam_2, gam_P, tau_lim);
//
//
//     for (i = 0; i < 3; i++) {
//         gam_V[i] = gam_V[i] / mu;
...
//
//     a1 = -1.0 / (((double)3.0) * pow(tau_lim, 2.0))
...
//                 * ((3.0 * pow(gam_1, 2.0) - pow(gam_V[i], 2.0))
//                        * (3.0 * pow(gam_2, 2.0) - pow(gam_V[i], 2.0)) * (gam_1 + gam_2)
//                    - 3.0 * gam_V[i]
//                          * (6.0 * pow(gam_1, 2.0) * pow(gam_2, 2.0)
//                             - pow(gam_V[i], 2.0) * (pow(gam_1, 2.0) + pow(gam_2, 2.0))))
//                 / (1.0 - pow(gam_V[i], 2.0) * pow(tau_lim, 2.0));
//
...
//
//         b1[i] = gam_1 * gam_2 * (3.0 * pow(gam_1, 2.0) - pow(gam_V[i], 2.0))
//                 * (3.0 * pow(gam_2, 2.0) - pow(gam_V[i], 2.0)) * pow(tau_lim, 2)
//                 / (gam_P * pow(gam_V[i], 2.0) * (pow(gam_V[i], 2.0) * pow(tau_lim, 2.0) - 1.0));
//
//         b2[i] = 3.0 * (gam_1 + gam_2) * pow(gam_V[i], 3.0)
//                 / ((3.0 * pow(gam_1, 2.0) - pow(gam_V[i], 2.0))
...
//             - pow((gam_1 * gam_2), 2.0) / sqrt(3.0 * gam_P)
//             - pow((gam_1 * gam_2), 3.0) / ((1.0 - gam_1) * (1.0 - gam_2) * (gam_1 + gam_2)));
//
...
//
//         D[i] = 1.0 / ((double)3.0) * pow((gam_1 * gam_2), 2.0) / gam_P * b0 * b1[i]
//                * (1.0 + b2[i] + b3[i]);
//
...
//     for (i = 0; i < 3; i++) {
//         summy +=
//             3.0 * Beta_V[i] * pow(Tmu, 4.0) / 4.0
//             * (C[i] + D[i] * exp(-tau[nlay - 1] / tau_lim) + E[i] * exp(-gam_V[i] * tau[nlay - 1]));
//     }
//
//     Tskin =
...
//         k_Ross_Freedman(
//             Tl[id * nlay + i + 1], sqrt(pl[id * nlay + i + 1] * pl[id * nlay + i]), met, kRoss[i]);
//
...
//         }
//         Tl[id * nlay + i] =
//             3.0 * pow(Tint, 4.0) / 4.0 * (tau[i] + A + B * exp(-tau[i] / tau_lim)) + summy;
//         Tl[id * nlay + i] = pow(Tl[id * nlay + i], (1.0 / 4.0));
//
//         // Convergence loop
//         for (j = 0; j < 5; j++) {
//             k_Ross_Freedman(sqrt(Tl[id * nlay + i + 1] * Tl[id * nlay + i]),
//                             sqrt(pl[id * nlay + i + 1] * pl[id * nlay + i]),
//                             met,
//                             kRoss[i]);
//
//             tau[i] = tau[i + 1] + kRoss[i] * (pl[id * nlay + i] - pl[id * nlay + i + 1]) / grav;
...
// This subroutine follows Parmentier & Guillot (2014, 2015) non-grey picket fence scheme
void Parmentier_IC_1D(const int nlay,
                      double*   pl,
                      double    Tint,
                      double    mu,
                      double    Tirr,
    ...
    for (i = 0; i < 3; i++) {
        a2[i] = pow(tau_lim, 2) / (gam_P * pow(gam_V[i], 2))
                * ((3.0 * pow(gam_1, 2) - pow(gam_V[i], 2))
                       * (3.0 * pow(gam_2, 2) - pow(gam_V[i], 2)) * (gam_1 + gam_2)
                   - 3.0 * gam_V[i]
                         * (6.0 * pow(gam_1, 2) * pow(gam_2, 2)
                            - pow(gam_V[i], 2) * (pow(gam_1, 2) + pow(gam_2, 2))))
                / (1.0 - pow(gam_V[i], 2) * pow(tau_lim, 2));

        a3[i] = -pow(tau_lim, 2) * (3.0 * pow(gam_1, 2) - pow(gam_V[i], 2))
                * (3.0 * pow(gam_2, 2) - pow(gam_V[i], 2)) * (Av2[i] + Av1[i])
                / (gam_P * pow(gam_V[i], 3) * (1.0 - pow(gam_V[i], 2) * pow(tau_lim, 2)));

        b1[i] = gam_1 * gam_2 * (3.0 * pow(gam_1, 2) - pow(gam_V[i], 2))
                * (3.0 * pow(gam_2, 2) - pow(gam_V[i], 2)) * pow(tau_lim, 2)
                / (gam_P * pow(gam_V[i], 2) * (pow(gam_V[i], 2) * pow(tau_lim, 2) - 1.0));

        b2[i] =
            3.0 * (gam_1 + gam_2) * pow(gam_V[i], 3)
            / ((3.0 * pow(gam_1, 2) - pow(gam_V[i], 2)) * (3.0 * pow(gam_2, 2) - pow(gam_V[i], 2)));
    ...
    b0 = 1.0
         / (gam_1 * gam_2 / (gam_1 - gam_2) * (At1 - At2) / 3.0
            - pow((gam_1 * gam_2), 2) / sqrtl(3.0 * gam_P)
            - pow((gam_1 * gam_2), 3) / ((1.0 - gam_1) * (1.0 - gam_2) * (gam_1 + gam_2)));

    A = 1.0 / 3.0 * (a0 + a1 * b0);
    B = -1.0 / 3.0 * pow((gam_1 * gam_2), 2) / gam_P * b0;

    for (i = 0; i < 3; i++) {
        C[i] = -1.0 / 3.0 * (b0 * b1[i] * (1.0 + b2[i] + b3[i]) * a1 + a2[i] + a3[i]);

        D[i] = 1.0 / 3.0 * pow((gam_1 * gam_2), 2) / gam_P * b0 * b1[i] * (1.0 + b2[i] + b3[i]);

        E[i] = (3.0 - pow((gam_V[i] / gam_1), 2)) * (3.0 - pow((gam_V[i] / gam_2), 2))
               / (9.0 * gam_V[i] * (pow((gam_V[i] * tau_lim), 2) - 1.0));
    }
...
// This subroutine follows Parmentier & Guillot (2014, 2015) non-grey picket fence scheme
void Parmentier_bilinear_interpolation_IC(int       id,
                                          const int nlay,
                                          double*   pl,
                                          double    Tint,
                                          double    mu,
    ...
    k_Ross_Freedman_bilinear_interpolation_polynomial_fit(Tskin,
                                                          pl[id * nlay + nlay - 1],
                                                          OpaTableTemperature,
                                                          OpaTablePressure,
                                                          OpaTableKappa,
                                                          kRoss[nlay - 1]);
    //k_Ross_Freedman(Tskin, pl[0], met, kRoss[i]);
    ...
    for (i = nlay - 2; i > -1; i--) {
        ...
        k_Ross_Freedman_bilinear_interpolation_polynomial_fit(
            Tl[id * nlay + i + 1],
            sqrt(pl[id * nlay + i + 1] * pl[id * nlay + i]),
            OpaTableTemperature,
            OpaTablePressure,
...
void adiabat_correction(int id, int nlay, double*(&Tl), double* pressure_h, double Gravit) {
    ...
    for (i = (nlay - 1); i > 1; i--) {
        ...
        gradrad[i] = (log10(Tl[i]) - log10(Tl[i - 1]))
                     / (log10(pressure_h[id * nlay + i]) - log10(pressure_h[id * nlay + i - 1]));
    ...
    if (iRC > -1) {
        for (i = iRC; i > 0; i--) {
            ...
            Tl[id * nlay + i - 1] =
                Tl[id * nlay + i]
                * pow((pressure_h[id * nlay + i - 1]) / pressure_h[id * nlay + i], gradad[i]);
        }
    }
}
...
void IC_adiabat_correction(int nlay, double*(&Tl), double* pressure_h, double Gravit, bool surface) {
    ...
    for (i = (nlay - 1); i > 1; i--) {
        ...
        gradrad[i] =
            (log10(Tl[i]) - log10(Tl[i - 1])) / (log10(pressure_h[i]) - log10(pressure_h[i - 1]));
        gradad[i] = ((double)0.32) - ((double)0.10) * Tl[i] / ((double)3000.0);
    }
...
void bottum_up_adiabat_correction(int id,
                                  int nlay,
                                  double*(&Tl),
                                  double* pressure_h,
                                  double  gravity,
    ...
    for (int lev = 1; lev < nlay ; lev++)
    {
        ...
        PT_actual = Tl[id * nlay + lev]*pow(P_Ref / pressure_h[id * nlay + lev], kappa);

        kappa = Rd_h[id * nlay + lev-1] / Cp_h[id * nlay + lev-1];
        PT_lower = Tl[id * nlay + lev-1]*pow(P_Ref / pressure_h[id * nlay + lev-1], kappa);
