        ...
        if (insol_avg == DIURNAL_AVG) {
            if (sim.Omega != mean_motion) {
                if (moon_irr) {
                    ...
                    Pday = 2 * M_PI / (2.0*sim.Omega); //what do i do in case of Omega < 0 ??
                }
                else {
    ...
    if (moon_irr_mode) 
    {
        ...
        } else
        {
            if (obliquity > 0.0)
            {
                ...
                if (isnan(pol_2_moon))
                {
                    log::printf("    pol_2_moon is NaN  \n");
                }
                if (isnan(phi_min))
                {
                    log::printf("    phi_min is NaN  \n");
                }
